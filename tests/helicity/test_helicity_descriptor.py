...
def test_handles_invalid_array(shape):
...
def test_handles_invalid_kernel_size(k):
    with pytest.raises(ValueError):
        heliq.helicity_descriptor(np.random.standard_normal((2, 2, 2)), kernel_size=k)


def test_handles_valid_kernel_size():
    heliq.helicity_descriptor(np.random.standard_normal((2, 2, 2)), kernel_size=3)
